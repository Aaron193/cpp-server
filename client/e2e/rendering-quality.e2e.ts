...
test('captures deterministic WebGL2 software-tier presentation facts', async ({
    page,
}, testInfo) => {
    ...
    expect(debug?.rendering.quality).toMatchObject({
        backend: 'webgl2',
        tier: 'software',
        resolutionScale: 0.65,
    })
    expect(debug?.rendering.quality.effectiveDpr).toBeLessThanOrEqual(0.75)
    expect(debug?.rendering.environment).toMatchObject({
        exposure: 1,
        toneMapping: 'aces',
        imageProcessing: 'linear-pbr-forward',
        shadowsEnabled: false,
    })
    expect(debug?.rendering.post).toMatchObject({
        postAA: 'fxaa',
        postProcessCount: 1,
        finalGradePasses: 0,
    })
    expect(debug?.rendering.world).toMatchObject({
        decorativeSources: 1,
    ...
    const profile = await page.evaluate(() =>
        (
            window as Window & { __arenaProfile?: () => unknown }
        ).__arenaProfile?.()
    )
    console.info(
        JSON.stringify({
            event: 'phase5_render_profile',
    ...
    await expect
        .poll(
            async () =>
                (await page.evaluate(() => window.__gameDebug?.()))?.rendering
                    .quality.canvasWidth
        )
        .toBe(1024)
    const resized = await page.evaluate(
        () => window.__gameDebug?.()?.rendering.quality
    )
