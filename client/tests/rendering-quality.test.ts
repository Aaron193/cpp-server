...
import {
    applyImportedTexturePolicy,
    type MutableAssetTextureFacts,
} from '../src/foundation/assets/GltfAssetRegistry'
import type { ClientMapGameplay } from '../src/foundation/assets/MapManifest'
import {
    configureImageProcessing,
    shadowPolicy,
} from '../src/foundation/rendering/EnvironmentLightingModule'
import {
    antialiasingPolicy,
    isSoftwareRenderer,
    renderTierProfile,
    resolutionPolicy,
    selectRenderTier,
    type RenderHardwareFacts,
} from '../src/foundation/rendering/RenderQualityModule'
import {
    WorldPresentation,
    decorationMarkers,
} from '../src/foundation/rendering/WorldPresentation'

function hardware(
    overrides: Partial<RenderHardwareFacts> = {}
): RenderHardwareFacts {
    return {
        backend: 'webgl2',
        renderer: 'Test GPU',
...
describe('render quality policy', () => {
    it('selects measured backend-aware tiers and honors deterministic overrides', () => {
        ...
        expect(selectRenderTier(hardware({ softwareRenderer: true }))).toBe(
            'software'
        )
        expect(selectRenderTier(hardware({ backend: 'webgpu' }), 'low')).toBe(
            'low'
        )
        expect(
            isSoftwareRenderer('ANGLE (Google, Vulkan SwiftShader device)')
        ).toBe(true)
    })
    ...
    it('uses actual sample support for AA and alpha-tested policy', () => {
        expect(antialiasingPolicy(4)).toEqual({
            mode: 'msaa',
            samples: 4,
            alphaTest: 'alpha-to-coverage',
        })
        expect(antialiasingPolicy(1)).toEqual({
            mode: 'fxaa',
            samples: 1,
    ...
    it('bounds shadows by tier, distance, and caster budget', () => {
        expect(shadowPolicy(renderTierProfile('high'), 72, 300)).toEqual({
            enabled: true,
            mapSize: 2048,
            casterCount: 240,
            casterBudget: 240,
        ...
        expect(shadowPolicy(renderTierProfile('low'), 72, 100)).toEqual({
            enabled: false,
            mapSize: 0,
            casterCount: 0,
            casterBudget: 0,
        ...
        expect(shadowPolicy(renderTierProfile('high'), 0, 5).enabled).toBe(
            false
        )
    })
...
describe('render asset and world presentation policy', () => {
    it('changes only texture sampling and preserves authored PBR material identity and values', () => {
        ...
        const facts: MutableAssetTextureFacts = {
            compressionPolicy: 'ktx2-basis-preferred-with-source-fallback',
            filteringPolicy: 'trilinear-mipmapped',
            importedContainers: 0,
            materials: 0,
            pbrMaterials: 0,
            textures: 0,
    ...
    it('uses bounded landmark decoration instances and LOD without collision data', () => {
        expect(
            decorationMarkers(gameplay, 2).map((marker) => marker.id)
        ).toEqual(['north', 'south'])
        ...
        const world = new WorldPresentation(
            scene,
            gameplay,
            renderTierProfile('medium'),
            []
        )
        expect(world.snapshot).toMatchObject({
            decorativeSources: 1,
            decorativeInstances: 2,
            lodLevels: 1,
            decorationBudget: 20,
        })
        expect(
            scene.meshes
                .filter((mesh) => mesh.name.startsWith('map-decoration/'))
                .every((mesh) => !mesh.isPickable)
        ).toBe(true)
        world.dispose()
        scene.dispose()
        engine.dispose()
    ...
    it('keeps the software fallback deliberately small and shadow-free', () => {
        ...
        const world = new WorldPresentation(
            scene,
            gameplay,
            renderTierProfile('software'),
            []
        )
        expect(world.snapshot).toMatchObject({
            decorativeInstances: 1,
            lodLevels: 0,
            decorationBudget: 2,
        })
        world.dispose()
