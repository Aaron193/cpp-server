...
describe('network handshake validation', () => {
    it('validates discovery, welcome, map, and the exact configuration bytes', async () => {
        ...
        const welcome: Welcome = { protocolVersion: PROTOCOL_VERSION, serverBuildId: 'dev', playerId: 7, tickRate: 60, snapshotRate: 20, map: { mapId: manifest.mapId, formatVersion: manifest.formatVersion, contentHash: manifest.contentHash }, configurationHash }
    ...
    it('rejects metadata mismatch and reserialized/tampered configuration JSON', async () => {
        ...
        const welcome: Welcome = { protocolVersion: PROTOCOL_VERSION, serverBuildId: 'dev', playerId: 1, tickRate: 60, snapshotRate: 20, map: { mapId: manifest.mapId, formatVersion: 1, contentHash: manifest.contentHash }, configurationHash: hash }
