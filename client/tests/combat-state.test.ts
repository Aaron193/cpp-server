...
describe('authoritative combat presentation model', () => {
    ...
    it('routes correlations, scores, rounds, feed, chat and clears all bounded state', () => {
        ...
        state.shot({ serverTick: 1, shooterId: 7, inputSequence: 9, actionId: 9, shotId: 4, weapon: Weapon.Rifle, origin: { x: 0, y: 1, z: 0 }, pelletEndPositions: [{ x: 0, y: 1, z: -80 }] })
        expect(state.eventsAfter(0).find((event) => event.kind === 'shot')).toMatchObject({ correlated: true })
        state.shot({ serverTick: 2, shooterId: 8, inputSequence: 9, actionId: 9, shotId: 5, weapon: Weapon.Rifle, origin: { x: 0, y: 1, z: 0 }, pelletEndPositions: [{ x: 0, y: 1, z: -80 }] })
        ...
        for (let id = 0; id < 200; id++) state.impact({ serverTick: id, shotId: id, pelletIndex: 0, position: { x: 0, y: 0, z: 0 }, normal: { x: 0, y: 1, z: 0 }, material: ImpactMaterial.World })
