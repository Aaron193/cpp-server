...
describe('offline gameplay input gating', () => {
    it('tracks normalized movement and consumes jump once', () => {
        ...
        expect(state.snapshot(true)).toEqual({ forward: 1, right: 1, jump: true, fire: false, reload: false, selectedWeapon: 1, scoreboard: false })
        expect(state.snapshot(true)).toEqual({ forward: 1, right: 1, jump: false, fire: false, reload: false, selectedWeapon: 1, scoreboard: false })
    })

    it('clears held input as soon as focus gating blocks gameplay', () => {
        const state = new InputState()
        state.keyDown('KeyW', true)
        expect(state.snapshot(false)).toEqual({ forward: 0, right: 0, jump: false, fire: false, reload: false, selectedWeapon: 1, scoreboard: false })
        expect(state.snapshot(true)).toEqual({ forward: 0, right: 0, jump: false, fire: false, reload: false, selectedWeapon: 1, scoreboard: false })
    })
