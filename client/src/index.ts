...
import './assets/styles/military.css'
...
declare global {
    interface Window {
        __gameDebug?: () =>
            | ReturnType<FoundationClient['debugSnapshot']>
            | undefined
    }
}
...
async function startGame(
    ...
    if (!gameContainer || !canvas || !hudRoot)
        throw new Error('Offline game shell is incomplete')
    gameContainer.classList.remove('hidden')
    const selectedMapId =
        server?.mapId ?? (import.meta.env.VITE_OFFLINE_MAP_ID || 'ironworks')
    ...
    try {
        ...
        let requestJoin:
            | (() => Promise<{ websocketUrl: string; ticket: string }>)
            | undefined
        if (server) {
            const apiBase =
            ...
            requestJoin = async () => {
                const response = await fetch(
                    `${apiBase}/servers/${encodeURIComponent(server.id)}/join`,
                    {
                        method: 'POST',
                        credentials: 'include',
                ...
                if (!response.ok || !body.websocketUrl || !body.ticket)
                    throw new Error(
                        body.error ?? 'Unable to obtain a game join ticket'
                    )
        ...
        const preferredBackend =
            backendValue === 'webgpu' || backendValue === 'webgl2'
                ? (backendValue as RenderingBackend)
                : 'auto'
        ...
        client = new FoundationClient({
            canvas,
            hudRoot,
            mapRoot: `/maps/${encodeURIComponent(selectedMapId)}`,
            rendering: { preferredBackend },
            renderQuality: {
                tier: renderTier,
                resolutionScale: settings.renderScale,
            },
            camera: { fieldOfViewRadians: (settings.fov * Math.PI) / 180 },
            movementTuning: {
                groundSpeed: 4.6,
                sprintSpeed: 6.6,
                crouchSpeed: 2.45,
            ...
            networking: server
                ? {
                      ...
                      server: {
                          websocketUrl:
                              join?.websocketUrl ?? server.websocketUrl,
        ...
        if (import.meta.env.DEV)
            window.__gameDebug = () => client?.debugSnapshot()
...
window.addEventListener('load', async () => {
    const homeScreen = new HomeScreen('home-screen', (server) => {
        void startGame(homeScreen, server)
    })
