...
export type CombatEvent =
    ...
    | {
          readonly id: number
          readonly kind: 'predicted-contact'
          readonly actionId: number
    ...
    | {
          ...
          readonly value: Damage
          readonly localHit: boolean
          readonly localDamage: boolean
      }
    ...
    | {
          readonly id: number
          readonly kind: 'round'
          readonly value: RoundTransition
      }
...
export class CombatPresentationState {
    private local: LocalCombatState = {
        playerId: null,
        health: null,
        weapon: Weapon.None,
        magazineAmmo: 0,
    ...
    private offlineInventory = new Map<
        Weapon,
    ...
    setPlayerId(playerId: number): void {
        this.local = { ...this.local, playerId }
    }
    ...
    selectOfflineWeapon(weapon: Weapon): void {
        ...
        this.offlineInventory.set(this.local.weapon, {
            magazineAmmo: this.local.magazineAmmo,
            reserveAmmo: this.local.reserveAmmo,
        })
        const magazineAmmo =
            INFANTRY_WEAPONS[
                weapon === Weapon.Shotgun ? Weapon.Shotgun : Weapon.Rifle
            ].magazineSize
        const ammo = this.offlineInventory.get(weapon) ?? {
            magazineAmmo,
            reserveAmmo: magazineAmmo * 4,
        }
        this.local = { ...this.local, weapon, ...ammo, reloading: false }
    }
    offlineFire(
        actionId: number,
        sequence: number,
        ...
        this.local = {
            ...this.local,
            magazineAmmo: Math.max(0, this.local.magazineAmmo - 1),
        }
        return true
    }
    beginOfflineReload(
        ...
        if (
            this.local.dead ||
            this.local.reloading ||
            this.local.magazineAmmo >= magazineSize ||
            this.local.reserveAmmo <= 0
        )
            return false
        this.local = { ...this.local, reloading: true }
        this.push({
            kind: 'local-reload',
            actionId,
            sequence: actionId,
    ...
    finishOfflineReload(magazineSize: number): void {
        if (!this.local.reloading) return
        const amount = Math.max(
            0,
            Math.min(
                magazineSize - this.local.magazineAmmo,
                this.local.reserveAmmo
            )
        )
        this.local = {
            ...this.local,
            magazineAmmo: this.local.magazineAmmo + amount,
            reserveAmmo: this.local.reserveAmmo - amount,
            reloading: false,
        }
    }
    ...
    localFire(
        actionId: number,
        sequenceOrWeapon: number | Weapon,
        weapon?: Weapon,
        nowMs = performance.now()
    ): void {
        const sequence = weapon === undefined ? actionId : sequenceOrWeapon
        const selected =
            weapon === undefined ? (sequenceOrWeapon as Weapon) : weapon
        this.addAction({
            actionId: actionId >>> 0,
    ...
    canLocalFire(weapon: Weapon): boolean {
        return (
            !this.local.dead &&
            !this.local.reloading &&
            this.local.weapon === weapon &&
            this.local.magazineAmmo > 0
        )
    }
    shot(value: ShotConfirmed): void {
        const correlated =
            value.shooterId === this.local.playerId &&
            this.pendingActions.has(value.actionId)
        this.push({ kind: 'shot', value, correlated })
    }
    actionResult(value: ActionResult, nowMs = performance.now()): void {
        const pending = this.pendingActions.get(value.actionId)
        if (pending && pending.kind === value.kind)
            this.pendingActions.delete(value.actionId)
        this.push({
            kind: 'action-result',
            value,
            latencyMs: pending ? Math.max(0, nowMs - pending.startedAtMs) : 0,
        })
    }
    predictedContact(actionId: number, position: Vec3): void {
        if (this.pendingActions.has(actionId))
            this.push({ kind: 'predicted-contact', actionId, position })
    }
    expireActions(nowMs = performance.now(), timeoutMs = 1500): number {
        let expired = 0
    ...
    impact(value: Impact): void {
        this.push({ kind: 'impact', value })
    }
    damage(value: Damage): void {
        this.push({
            kind: 'damage',
            value,
            localHit: value.sourceId === this.local.playerId,
            localDamage: value.targetId === this.local.playerId,
        })
    }
    ...
    respawn(value: Respawn): void {
        this.push({ kind: 'respawn', value })
    }
    score(value: ScoreChange): void {
        if (
            !this.scoreMap.has(value.playerId) &&
            this.scoreMap.size >= this.maxScores
        )
            this.scoreMap.delete(this.scoreMap.keys().next().value!)
        this.scoreMap.set(value.playerId, {
    ...
    round(value: RoundTransition): void {
        this.currentMatch = value.match
        this.push({ kind: 'round', value })
    }
    ...
    validateChat(text: string): string {
        ...
        if (new TextEncoder().encode(clean).length > LIMITS.maxChatBytes)
            throw new Error('Chat message is too long')
        return clean
    }
    /** Compatibility snapshot for tests/tools; frame loops should use forEachEventAfter. */
    eventsAfter(id: number): readonly CombatEvent[] {
        return this.eventQueue.toArray().filter((event) => event.id > id)
    }
    forEachEventAfter(id: number, visitor: (event: CombatEvent) => void): void {
        this.eventQueue.forEach((event) => {
            if (event.id > id) visitor(event)
        })
    }
    forEachScore(visitor: (row: ScoreRow) => void): void {
        for (const row of this.scores) visitor(row)
    }
    forEachKillFeed(visitor: (row: KillFeedRow) => void): void {
        this.feed.forEach(visitor)
    }
    forEachChatMessage(visitor: (row: ChatRow) => void): void {
        this.messages.forEach(visitor)
    }
    get localPlayer(): LocalCombatState {
        return this.local
    }
    get scores(): readonly ScoreRow[] {
        return [...this.scoreMap.values()].sort(
            (a, b) => b.score - a.score || a.playerId - b.playerId
        )
    }
    get killFeed(): readonly KillFeedRow[] {
        return this.feed.toArray()
    }
    get chatMessages(): readonly ChatRow[] {
    ...
    clear(): void {
        ...
        this.currentMatch = DEFAULT_MATCH
        this.eventId = 0
        this.scoreRevision++
        this.feedRevision++
        this.chatRevision++
    ...
    private addAction(action: PendingAction): void {
        if (action.actionId === 0)
            throw new RangeError('Action id zero is reserved')
        if (this.pendingActions.has(action.actionId))
            throw new Error(`Duplicate action id ${action.actionId}`)
        if (this.pendingActions.size >= 64) {
...
export function alignClientTick(previous: number, serverTick: number): number {
    ...
    return previous === 0 || (distance !== 0 && distance < 0x80000000)
        ? candidate
        : (previous + 1) >>> 0
}
