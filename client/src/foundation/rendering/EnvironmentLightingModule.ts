...
import type { FrameUpdate } from '../lifecycle'
...
export function configureImageProcessing(
...
export function shadowPolicy(
    profile: Pick<
        import('./RenderQualityModule').RenderTierProfile,
        'shadowMapSize' | 'shadowCasterBudget'
    >,
    ...
    const distance = Math.max(
        0,
        Number.isFinite(shadowDistance) ? shadowDistance : 0
    )
    const casterCount = Math.min(
        Math.max(0, eligibleCasters),
        profile.shadowCasterBudget
    )
    const enabled = profile.shadowMapSize > 0 && distance > 0 && casterCount > 0
    return {
        enabled,
        mapSize: enabled ? profile.shadowMapSize : 0,
        casterCount: enabled ? casterCount : 0,
        casterBudget: profile.shadowCasterBudget,
        distance,
    }
}
...
export class EnvironmentLightingModule implements ClientModule {
    ...
    private state: EnvironmentSnapshot = {
        applied: false,
        clearColor: DEFAULT_ENVIRONMENT.clearColor,
        sunDirection: DEFAULT_ENVIRONMENT.sunDirection,
        exposure: 1,
        contrast: 1,
        toneMapping: 'aces',
    ...
    apply(environment: MapEnvironment, meshes: readonly AbstractMesh[]): void {
        ...
        this.ambient.intensity = quality.profile.tier === 'software' ? 1 : 0.48
        this.ambient.groundColor = new Color3(0.12, 0.15, 0.2)
        this.sun = new DirectionalLight(
            'map-environment/sun',
            new Vector3(...environment.sunDirection),
            scene
        )
        this.sun.intensity = quality.profile.tier === 'software' ? 0.7 : 2.1

        const distance = Math.max(0, environment.shadowDistance)
        this.sun.position.copyFrom(
            this.sun.direction.scale(-Math.max(20, distance * 0.75))
        )
        ...
        if (policy.enabled) {
            ...
            this.shadow.filteringQuality =
                quality.profile.tier === 'high'
    ...
    start(): void {
        if (!this.context || !this.shadow || !this.sun) return
        ...
        if (profile.tier === 'high' || profile.tier === 'ultra') {
            ...
            csm.numCascades = profile.tier === 'ultra' ? 4 : 3
            csm.stabilizeCascades = true
            csm.lambda = 0.75
            csm.shadowMaxZ = this.state.shadowDistance
    ...
    update(frame: FrameUpdate): void {
        if (
            !this.shadow ||
            !this.context ||
            frame.elapsedSeconds < this.nextSelection
        )
            return
        this.nextSelection = frame.elapsedSeconds + 0.4
        const camera = this.context.services.get(CAMERA),
            distance = this.state.shadowDistance
        const sorted = this.casters
            .filter(
                (mesh) =>
                    mesh.getBoundingInfo().boundingBox.minimumWorld.y >= -0.6 ||
                    mesh.getBoundingInfo().boundingBox.maximumWorld.y > 1
            )
            .map((mesh) => ({
                mesh,
                d: Vector3.DistanceSquared(
                    mesh.getBoundingInfo().boundingBox.centerWorld,
                    camera.position
                ),
            }))
            .filter((v) => v.d < (distance + 60) * (distance + 60))
            .sort((a, b) => a.d - b.d)
            .slice(0, this.state.shadowCasterBudget)
            .map((v) => v.mesh)
        const map = this.shadow.getShadowMap()
        if (map) map.renderList = sorted
        if (!(this.shadow instanceof CascadedShadowGenerator) && this.sun)
            this.sun.position.copyFrom(
                camera.position.subtract(
                    this.sun.direction.scale(distance * 0.75)
                )
            )
        this.state = { ...this.state, shadowCasters: sorted.length }
    }
