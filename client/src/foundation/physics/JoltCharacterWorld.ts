...
export class JoltCharacterWorld {
    ...
    private readonly stanceShapeResources: Array<{
        result: any
        translated: any
        capsule: any
    }> = []
    ...
    constructor(
        ...
        for (let index = 0; index < collision.vertices.length; index += 3) {
            const vertex = new Jolt.Float3(
                collision.vertices[index],
                collision.vertices[index + 1],
                collision.vertices[index + 2]
            )
        ...
        for (let index = 0; index < collision.indices.length; index += 3) {
            const triangle = new Jolt.IndexedTriangle(
                ...
                collision.indices[index + 1],
                collision.indices[index + 2],
                0
            )
        ...
        const meshSettings = new Jolt.MeshShapeSettings(
            vertices,
            triangles,
            materials
        )
        ...
        if (!meshResult.IsValid())
            throw new Error(
                'Jolt rejected the authored collision triangle mesh'
            )
        ...
        this.staticBodyId = this.bodyInterface.CreateAndAddBody(
            bodySettings,
        ...
        const makeStanceShape = (
            stance: Stance,
            halfHeight: number,
            radius: number
        ): any => {
            const shapeOffset = new Jolt.Vec3(0, halfHeight + radius, 0)
            const capsuleSettings = new Jolt.CapsuleShapeSettings(
                halfHeight,
                radius
            )
            const translatedSettings = new Jolt.RotatedTranslatedShapeSettings(
                shapeOffset,
                identity,
                capsuleSettings
            )
            ...
            if (!result.IsValid())
                throw new Error(`Jolt rejected movement stance ${stance}`)
            ...
            this.stanceShapeResources.push({
                result,
                translated: translatedSettings,
                capsule: capsuleSettings,
            })
            Jolt.destroy(shapeOffset)
            return shape
        }
        const standingShape = makeStanceShape(
            Stance.Standing,
        ...
        makeStanceShape(
            Stance.Crouched,
            tuning.crouchCapsuleHalfHeight,
            tuning.crouchCapsuleRadius
        )
        makeStanceShape(
            Stance.Prone,
            tuning.proneCapsuleHalfHeight,
            tuning.proneCapsuleRadius
        )
        ...
        characterSettings.mBackFaceMode =
            Jolt.EBackFaceMode_CollideWithBackFaces
        ...
        this.broadPhaseFilter = new Jolt.DefaultBroadPhaseLayerFilter(
            this.joltInterface.GetObjectVsBroadPhaseLayerFilter(),
            CHARACTER_LAYER
        )
        this.objectLayerFilter = new Jolt.DefaultObjectLayerFilter(
        ...
        this.updateSettings.mStickToFloorStepDown.Set(
            0,
            -tuning.stickToFloorDistance,
            0
        )
    ...
    step(command: MovementCommand, dt: number): void {
        ...
        const mantleTarget =
            command.jump && !grounded && this.tuning.mantleEnabled
                ? this.findMantleTarget(position, command.yaw)
                : undefined
        const resolvedCommand = mantleTarget
            ? { ...command, mantleTarget }
            : command
        const result = stepMovementState(
            this.movementStateValue,
            resolvedCommand,
            {
                grounded,
                position: { ...position },
        ...
        if (
            nextState.stance !== this.appliedStance &&
            !this.applyStance(nextState.stance)
        )
            nextState = { ...nextState, stance: this.appliedStance }
        this.movementStateValue = nextState
        if (result.authoredPosition) {
            this.spawnVector.Set(
                result.authoredPosition.x,
                result.authoredPosition.y,
                result.authoredPosition.z
            )
        ...
        const locked =
            result.state.mode === MovementMode.Dashing ||
            result.state.mode === MovementMode.Sliding
        ...
        const velocity = {
            x: locked
                ? result.desiredHorizontal.x
                : approach(currentVelocity.x, result.desiredHorizontal.x),
            y: currentVelocity.y,
            z: locked
                ? result.desiredHorizontal.z
                : approach(currentVelocity.z, result.desiredHorizontal.z),
        }
        if (grounded && currentVelocity.y - groundY < 0.1) {
            velocity.y = groundY
            if (result.jump) velocity.y += this.tuning.jumpSpeed
        }
        velocity.y = Math.max(
            velocity.y - this.tuning.gravity * dt,
            -this.tuning.terminalVelocity
        )
        this.velocityVector.Set(velocity.x, velocity.y, velocity.z)
        this.character.SetLinearVelocity(this.velocityVector)
        this.character.ExtendedUpdate(
            dt,
            this.physicsSystem.GetGravity(),
            this.updateSettings,
            this.broadPhaseFilter,
    ...
    setState(
        ...
        if (movementState) {
            const restored = {
                ...movementState,
                dashDirection: { ...movementState.dashDirection },
                mantleStart: { ...movementState.mantleStart },
                mantleTarget: { ...movementState.mantleTarget },
            }
            if (this.applyStance(restored.stance))
                this.movementStateValue = restored
        }
        Object.assign(this.cachedPosition, position)
        Object.assign(this.cachedVelocity, velocity)
    }

    get position(): PhysicsPosition {
        const value = this.character.GetPosition()
        this.cachedPosition.x = value.GetX()
        this.cachedPosition.y = value.GetY()
        this.cachedPosition.z = value.GetZ()
        return this.cachedPosition
    }
    get velocity(): PhysicsPosition {
        const value = this.character.GetLinearVelocity()
        this.cachedVelocity.x = value.GetX()
        this.cachedVelocity.y = value.GetY()
        this.cachedVelocity.z = value.GetZ()
    ...
    get movementState(): MovementState {
        return {
            ...this.movementStateValue,
            dashDirection: { ...this.movementStateValue.dashDirection },
            mantleStart: { ...this.movementStateValue.mantleStart },
            mantleTarget: { ...this.movementStateValue.mantleTarget },
        }
    }
    ...
    /** Read-only static query for cosmetic weapon clearance. Returns a segment fraction. */
    probeStatic(
        origin: PhysicsPosition,
        displacement: PhysicsPosition
    ): number | undefined {
        return this.castFraction(origin, displacement)
    }

    private castFraction(
        origin: PhysicsPosition,
        displacement: PhysicsPosition
    ): number | undefined {
        const rayOrigin = new this.Jolt.RVec3(origin.x, origin.y, origin.z)
        const rayDirection = new this.Jolt.Vec3(
            displacement.x,
            displacement.y,
            displacement.z
        )
        ...
        const fraction = collector.HadHit()
            ? collector.mHit.mFraction
            : undefined
        this.Jolt.destroy(collector)
        this.Jolt.destroy(settings)
        this.Jolt.destroy(ray)
        this.Jolt.destroy(rayDirection)
        this.Jolt.destroy(rayOrigin)
        return fraction
    }

    private findMantleTarget(
        feet: PhysicsPosition,
        yaw: number
    ): PhysicsPosition | undefined {
        const forward = { x: Math.sin(yaw), y: 0, z: -Math.cos(yaw) }
        const right = { x: -forward.z, y: 0, z: forward.x }
        let nearest: number | undefined
        for (const lateral of [
            0,
            -0.6 * this.tuning.capsuleRadius,
            0.6 * this.tuning.capsuleRadius,
        ]) {
            const fraction = this.castFraction(
                {
                    x: feet.x + right.x * lateral,
                    y: feet.y + this.tuning.mantleMinHeight,
                    z: feet.z + right.z * lateral,
                },
                {
                    x: forward.x * this.tuning.mantleReach,
                    y: 0,
                    z: forward.z * this.tuning.mantleReach,
                }
            )
            if (
                fraction !== undefined &&
                (nearest === undefined || fraction < nearest)
            )
                nearest = fraction
        }
        if (nearest === undefined) return undefined
        const distance =
            nearest * this.tuning.mantleReach + this.tuning.capsuleRadius + 0.08
        const beyond = {
            x: feet.x + forward.x * distance,
            z: feet.z + forward.z * distance,
        }
        const downLength =
            this.tuning.mantleMaxHeight - this.tuning.mantleMinHeight + 0.16
        const downOrigin = {
            x: beyond.x,
            y: feet.y + this.tuning.mantleMaxHeight + 0.08,
            z: beyond.z,
        }
        const topFraction = this.castFraction(downOrigin, {
            x: 0,
            y: -downLength,
            z: 0,
        })
        ...
        const target = {
            x: beyond.x,
            y: downOrigin.y - downLength * topFraction + 0.025,
            z: beyond.z,
        }
        const height =
            2 * (this.tuning.capsuleRadius + this.tuning.capsuleHalfHeight)
        for (const offset of [
            { x: 0, z: 0 },
            {
                x: right.x * this.tuning.capsuleRadius,
                z: right.z * this.tuning.capsuleRadius,
            },
            {
                x: -right.x * this.tuning.capsuleRadius,
                z: -right.z * this.tuning.capsuleRadius,
    ...
    dispose(): void {
        ...
        for (const resource of this.stanceShapeResources) {
            ...
            // RotatedTranslatedShapeSettings owns the inner settings through Ref<>;
            // destroying it releases the capsule. A second destroy corrupts the WASM heap.
        }
