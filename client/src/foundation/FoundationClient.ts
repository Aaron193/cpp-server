import { ConquestModule } from './conquest/ConquestModule'
...
import { ModuleLifecycle, ServiceRegistry, type FrameUpdate } from './lifecycle'
...
import {
    RenderQualityModule,
    type RenderQualityOverride,
} from './rendering/RenderQualityModule'
...
export class FoundationClient {
    ...
    constructor(private readonly options: FoundationClientOptions) {
        ...
        const reducedTierRequested =
            options.renderQuality?.tier === 'low' ||
            options.renderQuality?.tier === 'software'
        const renderingOptions: EngineFactoryOptions = {
            ...options.rendering,
            antialias:
                options.rendering?.antialias ??
                (reducedTierRequested ? false : undefined),
        }
        this.lifecycle = new ModuleLifecycle([
            ...
            new AimingModule(),
            new FirstPersonCameraModule({
                fieldOfViewRadians: options.camera?.fieldOfViewRadians,
            }),
    ...
    update(frame: FrameUpdate): void {
        const engine = this.services.get(ENGINE)
        engine.beginFrame()
        try {
    ...
    debugSnapshot(): {
        ...
        readonly conquest: ReturnType<FoundationClient['tacticalSnapshot']>
        readonly localWeapon: number
        readonly localMovement: {
        ...
        readonly rendering: {
            readonly quality: ReturnType<
                FoundationClient['renderingSnapshot']
            >['quality']
        ...
        return {
            ...
            localPosition: { ...this.services.get(PHYSICS).position },
            conquest: this.tacticalSnapshot(),
            ...
            meshes: scene.meshes
                .filter((mesh) =>
                    /^(remote-|viewmodel\/|tracer\/|muzzle\/)/.test(mesh.name)
                )
                .map((mesh) => ({
                    name: mesh.name,
    ...
    private tacticalSnapshot() {
        return this.services.get(NETWORKING).conquest
    }
