...
export class CameraRigController {
    ...
    hardReset(): void {
        ...
        this.previousFeetY = undefined; this.previousGrounded = true; this.currentFov = this.targetFov = this.baseFov
    }
    update(input: CameraRigInput, dt: number): CameraRigPose {
        ...
        return {
            position: {
                ...
                y: input.predictedFeet.y + input.eyeHeight + input.correctionResidual.y + this.stepOffset + this.crouchOffset + this.landingOffset + bobY,
            ...
            pitch: input.simulationPitch + this.damagePitch + this.recoilPitch,
            fov: this.currentFov,
        }
    }
}
