...
export class EntityViewsModule implements ClientModule {
    ...
    initialize(context: ClientModuleContext): void {
        ...
        for (const mesh of [this.playerTemplate, this.playerLodTemplate, this.weaponTemplate]) { mesh.material = this.sharedMaterial; mesh.isPickable = false; mesh.alwaysSelectAsActiveMesh = false; mesh.setEnabled(false) }
    }
    ...
    applyRemotePlayer(entity: EntityRecord): void {
        ...
        if (!view) {
            ...
            weapon.parent = mesh; weapon.position.set(.34, .25, -.12)
    ...
    dispose(): void {
        ...
        this.playerTemplate?.dispose(); this.playerLodTemplate?.dispose(); this.weaponTemplate?.dispose(); this.sharedMaterial?.dispose()
        this.playerTemplate = undefined; this.playerLodTemplate = undefined; this.weaponTemplate = undefined; this.sharedMaterial = undefined
