...
} from '../assets/CollisionMesh'
import {
    loadMapGameplay,
    loadMapManifest,
    type ClientMapGameplay,
    type ClientMapManifest,
} from '../assets/MapManifest'
import type {
    ClientModule,
    ClientModuleContext,
    FrameUpdate,
...
import {
    WorldPresentation,
    type WorldPresentationSnapshot,
} from '../rendering/WorldPresentation'
...
function matchingBounds(
    manifest: ClientMapManifest,
    collision: CollisionMeshData
): boolean {
    return [...manifest.worldBounds.min, ...manifest.worldBounds.max].every(
        (value, index) =>
            Math.abs(
                value -
                    [...collision.bounds.min, ...collision.bounds.max][index]
            ) < 1e-4
    )
}
...
export class MapModule implements ClientModule {
    ...
    async initialize(context: ClientModuleContext): Promise<void> {
        ...
        const manifest = await loadMapManifest(
            joinAsset(this.mapRoot, 'manifest.json')
        )
        const gameplay = await loadMapGameplay(
            joinAsset(this.mapRoot, manifest.assets.gameplay ?? ''),
            manifest
        )
        const manifestLoaded = performance.now()
        const collision = await loadCollisionMesh(
            joinAsset(this.mapRoot, manifest.assets.collision)
        )
        const collisionLoaded = performance.now()
        if (!matchingBounds(manifest, collision))
            throw new Error('Manifest and collision world bounds do not match')

        const scene = context.services.get(SCENE)
        const container = await context.services
            .get(ASSETS)
            .importContainer(joinAsset(this.mapRoot, manifest.assets.render))
        ...
        // Each compiled chunk occurs once. Clone its node while sharing geometry/materials;
        // instances of a detached container source do not receive newly created scene lights.
        ...
        const mapMeshes = scene.meshes.filter(
        ...
        for (const mesh of mapMeshes) {
            ...
            if (mesh instanceof Mesh && mesh.name.includes('render-grass')) {
                const tier = context.services.get(RENDER_QUALITY).profile.tier
                mesh.addLODLevel(
                    tier === 'software'
                        ? 30
                        : tier === 'low'
        ...
        this.worldPresentation = new WorldPresentation(
            scene,
            gameplay,
            context.services.get(RENDER_QUALITY).profile,
            mapMeshes
        )
        // Collision visualization is created on demand, avoiding a second full-map vertex allocation.
    ...
    update(_frame: FrameUpdate): void {
        if (this.context?.services.get(INPUT).consumeCollisionDebugToggle())
    ...
    private createDebugOverlay(
        ...
        for (let index = 0; index < collision.indices.length; index += 3) {
            const triangle = [0, 1, 2].map((corner) => {
                ...
                return new Vector3(
                    collision.vertices[vertex],
                    collision.vertices[vertex + 1],
                    collision.vertices[vertex + 2]
                )
            })
        ...
        const spawnMaterial = new StandardMaterial(
            'spawn-debug-material',
            scene
        )
    ...
    setDebugVisible(visible: boolean): void {
        if (
            ...
            this.createDebugOverlay(this.collision, this.gameplay)
    ...
    get mapManifest(): ClientMapManifest | undefined {
        return this.manifest
    }
    get mapGameplay(): ClientMapGameplay | undefined {
        return this.gameplay
    }
    /** Radar and navigation stay unfetched until a future HUD/bot consumer asks. */
    loadRadar(): Promise<Uint8Array | null> {
        return this.loadOptionalAsset('radar')
    }
    loadNavigation(): Promise<Uint8Array | null> {
        return this.loadOptionalAsset('navigation')
    }
    private loadOptionalAsset(
        kind: 'radar' | 'navigation'
    ): Promise<Uint8Array | null> {
        const existing = this.lazyAssets.get(kind)
        if (existing) return existing
        const pending = (async () => {
            const manifest = this.manifest,
                name = manifest?.assets[kind]
            if (!manifest || !name) return null
            const response = await fetch(joinAsset(this.mapRoot, name))
            if (!response.ok)
                throw new Error(
                    `Unable to load map ${kind} (${response.status})`
                )
            ...
            if (actual !== manifest.assetHashes[name])
                throw new Error(`Map ${kind} asset hash mismatch`)
            return bytes
        })()
        this.lazyAssets.set(kind, pending)
        return pending
    }
    /** Development baseline timings for the current staged map load. */
    get loadMetrics(): Readonly<typeof this.loadTiming> {
        return this.loadTiming
    }
    get presentationMetrics(): WorldPresentationSnapshot {
        return (
            this.worldPresentation?.snapshot ?? {
