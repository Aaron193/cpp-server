...
import {
    ...
    validateWelcome,
    type ServerDiscoveryDescriptor,
} from './Handshake'
import {
...
export type HardSyncReason =
    ...
    | 'map-transition'
    | 'history-overflow'
    | 'impossible-acknowledgement'
    | 'excessive-clock-discontinuity'
    | 'jolt-world-rebuild'
...
export interface NetworkingOptions {
    readonly server?: ServerDiscoveryDescriptor
    readonly clientBuildId?: string
    readonly accessToken?: string
    readonly joinTicketProvider?: () => Promise<{
...
export interface NetworkMetrics {
    ...
    readonly hardSyncCount: number
    readonly hardSyncReason: HardSyncReason | null
    readonly interpolationMode: 'none' | RemoteSample['mode']
    readonly interpolationDelayMs: number
    readonly interpolationUnderflows: number
    readonly interpolationOverflows: number
    readonly interpolatedSamples: number
    readonly extrapolatedSamples: number
    readonly frozenSamples: number
...
export const DEFAULT_RECONCILIATION_TUNING: ReconciliationTuning =
    Object.freeze({
...
export class NetworkingModule implements ClientModule {
    ...
    private offlineReloadUntil = 0
    private offlineTriggerDown = false
    private reloadRevision = 0
    deploy(spawnId: string, weapon: Weapon): void {
        if (this.status === 'connected')
            this.transport.send(
                encodeMessage({
                    type: MessageType.Deploy,
                    payload: { spawnId, weapon },
                })
            )
    }
    ...
    private readonly metricsState: {
        -readonly [Key in keyof NetworkMetrics]: NetworkMetrics[Key]
    } = {
        rttMs: 0,
        jitterMs: 0,
        snapshotAgeMs: 0,
    ...
    private impairment: SyntheticImpairment = {
        latencyMs: 0,
        jitterMs: 0,
        stalled: false,
    }
    ...
    constructor(private readonly options: NetworkingOptions = {}) {
        ...
        this.accumulator = new FixedStepAccumulator(
            1 / 60,
            0.25,
            options.maxCatchUpSteps ?? 5
        )
        this.history = new PredictionHistory(
            options.predictionHistoryCapacity ?? 256
        )
        this.reconciliationTuning = Object.freeze({
            ...DEFAULT_RECONCILIATION_TUNING,
    ...
    initialize(context: ClientModuleContext): void {
        ...
        if (this.options.server)
            context.services.get(PHYSICS).setExternalDrive(true)
        else this.combat.initializeOffline()
    }
    start(): void {
        if (this.options.server) this.connect()
    }
    ...
    update(frame: FrameUpdate): void {
        ...
        if (this.status === 'reconnecting' && now >= this.reconnectAtMs)
            this.connect()
        if (!this.context) return
        if (this.status === 'offline') {
            this.updateOffline(now)
            return
        }
        if (this.status !== 'connected' || !this.welcome) return
        this.sendPingIfDue(now)
        if (!this.clientTickAligned) {
            for (const text of this.context.services
                .get(INPUT)
                .consumeChatMessages())
                this.sendChat(text)
        ...
        this.accumulator.consume(frame.deltaSeconds, (dt) => {
            ...
            const selectedWeapon =
                snapshot.selectedWeapon === 2 ? Weapon.Shotgun : Weapon.Rifle
            const cosmeticInterval =
                1000 / this.weaponDefinitions[selectedWeapon].fireRate
            let fireActionId = 0,
                reloadActionId = 0
            ...
            if (
                snapshot.fire &&
                physics.canFire &&
                this.combat.canLocalFire(selectedWeapon) &&
                now - this.lastLocalFireAtMs >= cosmeticInterval
            ) {
                this.lastLocalFireAtMs = now
                fireActionId = this.nextActionId()
                this.combat.localFire(
                    fireActionId,
                    sequence,
                    selectedWeapon,
                    now
                ...
                this.context.services
                    .optional(AIMING)
                    ?.predictShot(fireActionId)
            }
            if (snapshot.reload) {
                reloadActionId = this.nextActionId()
                this.combat.localReload(
                    reloadActionId,
                    sequence,
                    selectedWeapon,
                    now
            ...
            const command: InputCommand = {
                ...
                moveX: snapshot.right * scale,
                moveY: -snapshot.forward * scale,
                buttonFlags:
                    (snapshot.jump ? JUMP_BUTTON : 0) |
                    (fireActionId ? FIRE_BUTTON : 0) |
                    (reloadActionId ? RELOAD_BUTTON : 0) |
                    (snapshot.sprint ? SPRINT_BUTTON : 0) |
                    (snapshot.crouch ? CROUCH_BUTTON : 0) |
                    (snapshot.prone ? PRONE_BUTTON : 0) |
                    (snapshot.dash ? DASH_BUTTON : 0) |
                    (input.aiming ? ADS_BUTTON : 0),
                fireActionId,
                reloadActionId,
            ...
            const pushed = this.history.push({
                command,
                position: { x: position.x, y: position.y, z: position.z },
                velocity: { x: velocity.x, y: velocity.y, z: velocity.z },
                movementState: physics.movementState,
                sentAtMs: now,
            })
        ...
        if (this.accumulator.lastDroppedSeconds > 0) {
            this.droppedSimulationTimeMs +=
                this.accumulator.lastDroppedSeconds * 1000
            this.requestHardSync('excessive-clock-discontinuity')
        }
        for (
            let offset = 0;
            offset < commands.length;
            offset += LIMITS.maxInputCommands
        ) {
            this.transport.send(
                encodeMessage({
    ...
    private updateOffline(now: number): void {
        ...
        const input = this.context.services.get(INPUT)
        const weapon =
            input.selectedWeapon === 2 ? Weapon.Shotgun : Weapon.Rifle
        if (this.combat.localPlayer.weapon !== weapon)
            this.offlineReloadUntil = 0
        this.combat.selectOfflineWeapon(weapon)
        const definition = this.weaponDefinitions[weapon]
        if (this.offlineReloadUntil && now >= this.offlineReloadUntil) {
            this.combat.finishOfflineReload(definition.magazineSize)
            this.offlineReloadUntil = 0
        }
        if (this.reloadRevision !== input.reloadRevision) {
            this.reloadRevision = input.reloadRevision
            if (
                this.combat.beginOfflineReload(
                    this.nextActionId(),
                    weapon,
                ...
                this.offlineReloadUntil = now + definition.reloadTime * 1000
        }
        const wasDown = this.offlineTriggerDown
        this.offlineTriggerDown = input.firing
        if (!definition.automatic && wasDown) return
        const interval = 1000 / definition.fireRate
        if (
            !input.firing ||
            !this.context.services.get(PHYSICS).canFire ||
            now - this.lastLocalFireAtMs < interval
        )
            return
        this.lastLocalFireAtMs = now
    ...
    setSyntheticImpairment(impairment: SyntheticImpairment): void {
        ...
        if (this.transport instanceof BrowserWebSocketTransport)
            this.transport.setImpairment(impairment)
    }
    get syntheticImpairment(): SyntheticImpairment {
    ...
    get tickRate(): number {
        return this.welcome?.tickRate ?? 60
    }
    get serverTickNow(): number | undefined {
        ...
        return (
            (this.latestServerTick +
                Math.floor(
                    ((now - this.lastSnapshotAtMs) * this.tickRate) / 1000
                )) >>>
            0
        )
    }
    matchCountdownSeconds(phaseEndsAtTick: number): number {
        ...
        return remainingTicks < 0x80000000
            ? Math.ceil(remainingTicks / this.tickRate)
            : 0
    }
    get visualCorrection(): Readonly<{ x: number; y: number; z: number }> {
        return this.visualResidual
    }
    ...
    sendChat(text: string): boolean {
        if (this.status !== 'connected') {
            this.detail = 'Chat unavailable while disconnected'
            return false
        }
        try {
            ...
            this.detail = errorMessage(error)
            return false
        }
    }
    get metrics(): NetworkMetrics {
        ...
        value.rttMs = clock?.sampleCount ? clock.rttMs : this.rttMs
        value.jitterMs = clock?.sampleCount ? clock.deviationMs : this.jitterMs
        value.clockOffsetMs = clock?.sampleCount ? clock.offsetMs : 0
        value.clockConfidence = clock?.confidence ?? 0
        value.clockAgeMs = clock?.sampleCount ? clock.ageMs : 0
        value.snapshotAgeMs = this.lastSnapshotAtMs
            ? Math.max(0, performance.now() - this.lastSnapshotAtMs)
            : 0
        value.correctionMagnitude = this.correctionMagnitude
        value.pendingInputs = this.history.size
        value.remotePlayers = this.timelines?.size ?? 0
        value.snapshotBytes = this.snapshotBytes
        value.correctionRevision = this.correctionRevision
        const offlineDroppedTimeMs =
            this.status === 'offline'
                ? (this.context?.services.get(PHYSICS)
                      .droppedSimulationTimeMs ?? 0)
                : 0
        value.droppedSimulationTimeMs =
            this.droppedSimulationTimeMs + offlineDroppedTimeMs
        value.replaySteps = this.replaySteps
        value.replayTimeMs = this.replayTimeMs
        value.hardSyncCount = this.hardSyncCount
        value.hardSyncReason = this.hardSyncReason
        const remoteTelemetry = this.timelines?.telemetry
        value.interpolationMode = this.interpolationMode
        value.interpolationDelayMs = this.interpolationDelay?.delayMs ?? 0
        value.interpolationUnderflows = remoteTelemetry?.underflows ?? 0
        value.interpolationOverflows = remoteTelemetry?.overflows ?? 0
        value.interpolatedSamples = remoteTelemetry?.interpolatedSamples ?? 0
        value.extrapolatedSamples = remoteTelemetry?.extrapolatedSamples ?? 0
        value.frozenSamples = remoteTelemetry?.frozenSamples ?? 0
        value.transportBufferedBytes = this.transport.bufferedBytes ?? 0
        return value
    }
    forEachCorrection(visitor: (value: number, index: number) => void): void {
        this.corrections.forEach(visitor)
    }
            ...
            void this.options
                ...
                .then((join) => {
                    ...
                    if (!/^wss?:\/\//.test(join.websocketUrl) || !join.ticket)
                        throw new Error(
                            'Join-ticket provider returned invalid connection data'
                        )
    ...
    private connectTransport(): void {
        ...
        if (!manifest) {
            this.fail('Loaded map manifest is unavailable')
            return
        }
            ...
            this.transport.connect(this.websocketUrl, {
                    ...
                    this.transport.send(
                        encodeMessage({
                            ...
                            payload: {
                                ...
                                clientBuildId:
                                    this.options.clientBuildId ?? 'dev',
                                supportedMapFormat: manifest.formatVersion,
                ...
                message: (data) => {
                    this.messageChain = this.messageChain
                        .then(() => this.handlePacket(data, generation))
                        .catch((error) => {
                            if (generation === this.sessionGeneration)
                                this.handleDecoderFailure(errorMessage(error))
                        })
                },
                error: (message) => {
                    if (generation === this.sessionGeneration)
        ...
        while (offset < data.length) {
            ...
            if (!this.welcome) {
                if (message.type !== MessageType.Welcome)
                    throw new Error('Expected Welcome before game messages')
                const manifest = this.context?.services.get(ARENA).mapManifest
                if (!manifest || !this.options.server)
                    throw new Error('Handshake context disappeared')
                validateWelcome(message.payload, {
                    clientBuildId: this.options.clientBuildId ?? 'dev',
                    discovery: this.options.server,
                    manifest,
                })
                ...
                this.interpolationDelay = new AdaptiveInterpolationDelay(
                    message.payload.snapshotRate
                )
            ...
            if (this.status !== 'connected') {
                if (message.type !== MessageType.Configuration)
                    throw new Error(
                        'Expected Configuration immediately after Welcome'
                    )
                this.weaponDefinitions = parseWeaponDefinitions(
                    message.payload.configurationJson
                )
            ...
            switch (message.type) {
                case MessageType.Snapshot:
                    this.snapshotBytes = envelope.nextOffset - envelopeOffset
                    this.acceptSnapshot(message.payload)
                    break
                case MessageType.SnapshotDelta:
                    this.snapshotBytes = envelope.nextOffset - envelopeOffset
                    this.acceptSnapshotDelta(message.payload)
                    break
                case MessageType.Pong:
                    this.acceptPong(message.payload, performance.now())
                    break
                case MessageType.Spawn:
                    this.acceptEntity(
                        message.payload.serverTick,
                        publicStateToEntityRecord(message.payload.entity)
                    )
                    break
                case MessageType.Remove:
                    this.removeRemote(entityHandleKey(message.payload.handle))
                    break
                case MessageType.ShotConfirmed:
                    this.combat.shot(message.payload)
                    break
                case MessageType.ActionResult:
                    this.combat.actionResult(message.payload)
                    if (message.payload.kind === 1)
                        this.context?.services
                            .optional(AIMING)
                            ?.resolveShot(
                ...
                case MessageType.Impact:
                    this.combat.impact(message.payload)
                    break
                case MessageType.Damage:
                    this.combat.damage(message.payload)
                    break
                case MessageType.Death:
                    this.combat.death(message.payload)
                    break
                case MessageType.Respawn:
                    ...
                    if (message.payload.playerId === this.welcome.playerId) {
                        ...
                        this.context?.services
                            .get(INPUT)
                            .angles.set(message.payload.bodyYaw, 0)
                    }
                    break
                case MessageType.ScoreChange:
                    this.combat.score(message.payload)
                    break
                case MessageType.RoundTransition:
                    this.combat.round(message.payload)
                    break
                case MessageType.ConquestState:
                    this.conquest = message.payload
                    break
                case MessageType.Chat:
                    this.combat.chat(message.payload)
                    break
            }
        }
    }

    private async acceptConfiguration(
        ...
        if (generation !== this.sessionGeneration) return
        await this.context.services
            .get(PHYSICS)
            .applyAuthoritativeTuning(tuning)
        this.context.services.optional(AIMING)?.configure(tuning.aiming)
        ConfigManager.resetConfig()
        ConfigManager.setConfig(tuning)
        if (generation !== this.sessionGeneration) return
    ...
    private acceptSnapshot(snapshot: Snapshot): void {
        ...
        if (
            this.latestServerTick !== undefined &&
            !isSequenceNewer(snapshot.serverTick, this.latestServerTick)
        )
            return
        const now = performance.now()
        this.interpolationDelay?.observeArrival(
            now,
            this.clock?.state(now).deviationMs ?? 0
        )
        if (this.previousSnapshotAtMs) {
            const actual = now - this.previousSnapshotAtMs,
                expected = 1000 / this.welcome.snapshotRate
        ...
        if (initialSnapshot) {
            this.clientTick = alignClientTick(
                this.clientTick,
                snapshot.serverTick
            )
        ...
        const local = snapshot.entities.find(
            (entity) => entity.entityId === this.welcome?.playerId
        )
        if (local) {
            if (initialSnapshot)
                this.context.services
                    .get(INPUT)
                    .angles.set(local.bodyYaw, local.aimPitch)
            this.reconcile(local, snapshot.lastProcessedInputSequence, now)
        }
        for (const entity of snapshot.entities)
            if (entity.entityId !== this.welcome.playerId)
                this.acceptEntity(snapshot.serverTick, entity)
    }

    private acceptSnapshotDelta(snapshot: SnapshotDelta): void {
        if (!this.context || !this.welcome) return
        if (
            entityHandleKey(snapshot.local.handle) !==
            entityHandleKey(this.welcome.playerHandle)
        )
        ...
        if (initialSnapshot) {
            ...
            this.context.services
                .get(INPUT)
                .angles.set(snapshot.local.bodyYaw, snapshot.local.aimPitch)
        }
        this.combat.acceptAuthoritative(snapshot.local, snapshot.match)
        this.context.services
            .optional(AIMING)
            ?.reconcile(snapshot.local.weaponState)
        const authoritative: EntityRecord = {
            entityId: entityHandleKey(snapshot.local.handle),
            kind: EntityKind.Player,
            position: snapshot.local.position,
            velocity: snapshot.local.velocity,
            bodyYaw: snapshot.local.bodyYaw,
            aimPitch: snapshot.local.aimPitch,
            grounded: snapshot.local.grounded,
            stateFlags: snapshot.local.stateFlags,
            stance: snapshot.local.movementState.stance,
            movementMode: snapshot.local.movementState.mode,
        ...
        this.reconcile(
            ...
            now,
            snapshot.local.movementState
        )
        for (const entity of applied.createdOrUpdated)
            this.acceptEntity(snapshot.serverTick, entity)
    ...
    private reconcile(
        ...
        if (
            acknowledgement.status === 'history-overflow' ||
            acknowledgement.status === 'impossible'
        ) {
            const reason: HardSyncReason =
                acknowledgement.status === 'history-overflow'
                    ? 'history-overflow'
                    : 'impossible-acknowledgement'
            this.hardSyncState(
        ...
        if (
            ...
            (this.clock?.state(now).sampleCount ?? 0) === 0
        ) {
            ...
            this.rttMs =
                this.rttMs === 0
                    ? sampleRtt
                    : this.rttMs + (sampleRtt - this.rttMs) * 0.15
            if (this.lastRttMs)
                this.jitterMs +=
                    (Math.abs(sampleRtt - this.lastRttMs) - this.jitterMs) * 0.1
            this.lastRttMs = sampleRtt
        }
        physics.setAuthoritativeState(
            authoritative.position,
            authoritative.velocity,
            movementState
        )
        ...
        const correctionX = after.x - beforeX,
            correctionY = after.y - beforeY,
            correctionZ = after.z - beforeZ
        this.recordCorrection(
            beforeX,
            beforeY,
            beforeZ,
        ...
        if (
            horizontalCorrection >
                this.reconciliationTuning.horizontalHardSnapMeters ||
            Math.abs(correctionY) >
                this.reconciliationTuning.verticalHardSnapMeters
        ) {
            this.history.clear()
            this.clearVisualResidual()
        ...
        const residualX = shownBeforeX - after.x,
            residualZ = shownBeforeZ - after.z
        const residualHorizontal = Math.hypot(residualX, residualZ)
        const horizontalScale =
            residualHorizontal >
            this.reconciliationTuning.maxHorizontalVisualCorrectionMeters
                ? this.reconciliationTuning
                      .maxHorizontalVisualCorrectionMeters / residualHorizontal
                : 1
        this.visualResidual.x = residualX * horizontalScale
        this.visualResidual.z = residualZ * horizontalScale
        this.visualResidual.y = Math.max(
            -this.reconciliationTuning.maxVerticalVisualCorrectionMeters,
            Math.min(
                this.reconciliationTuning.maxVerticalVisualCorrectionMeters,
                shownBeforeY - after.y
            )
        )
    }
    ...
    private recordCorrection(
        ...
        this.correctionMagnitude = Math.hypot(
            afterX - beforeX,
            afterY - beforeY,
            afterZ - beforeZ
        )
    ...
    private movementCommand(command: InputCommand) {
        ...
        return {
            forward: -command.moveY,
            right: command.moveX,
            jump: Boolean(command.buttonFlags & JUMP_BUTTON),
            yaw: command.yaw,
            sprint: Boolean(command.buttonFlags & SPRINT_BUTTON),
            crouch: Boolean(command.buttonFlags & CROUCH_BUTTON),
            prone: Boolean(command.buttonFlags & PRONE_BUTTON),
            dash: Boolean(command.buttonFlags & DASH_BUTTON),
            ads: Boolean(command.buttonFlags & ADS_BUTTON),
            selectedWeapon: command.selectedWeapon,
            aimProgress: aim?.aimProgress ?? 0,
            adsMoveMultiplier: aim?.adsMoveMultiplier ?? 1,
        }
    }

    private hardSyncState(
        position: { x: number; y: number; z: number },
        velocity: { x: number; y: number; z: number },
        reason: HardSyncReason,
        movementState: MovementState = createMovementState()
    ): void {
        this.context?.services
            .get(PHYSICS)
            .setAuthoritativeState(position, velocity, movementState)
        this.history.clear()
        this.accumulator.reset()
        this.clearVisualResidual()
    ...
    private recordHardSync(reason: HardSyncReason): void {
        this.hardSyncReason = reason
        this.hardSyncCount++
    }
    private clearVisualResidual(): void {
        this.visualResidual.x = 0
        this.visualResidual.y = 0
        this.visualResidual.z = 0
    }
    private decayVisualResidual(deltaSeconds: number): void {
        if (!Number.isFinite(deltaSeconds) || deltaSeconds <= 0) return
        const decay = Math.pow(
            0.5,
            deltaSeconds / this.reconciliationTuning.correctionHalfLifeSeconds
        )
        this.visualResidual.x *= decay
        this.visualResidual.y *= decay
        this.visualResidual.z *= decay
        if (
            Math.hypot(
                this.visualResidual.x,
                this.visualResidual.y,
                this.visualResidual.z
            ) < 1e-5
        )
            this.clearVisualResidual()
    }
    ...
    private acceptEntity(serverTick: number, entity: EntityRecord): void {
        if (
            !this.welcome ||
            entity.entityId === entityHandleKey(this.welcome.playerHandle) ||
    ...
    private updateRemoteViews(now: number): void {
        if (
            ...
            return
        const estimatedTick =
            this.clock?.estimatedServerTick(now) ??
            (this.latestServerTick +
                Math.floor(
                    ((now - this.lastSnapshotAtMs) * this.welcome.tickRate) /
                        1000
                )) >>>
                0
        const interpolationTicks = Math.round(
            ((this.interpolationDelay?.delayMs ?? 100) *
                this.welcome.tickRate) /
                1000
        )
        ...
        this.timelines.forEachSample(targetTick, (_entityId, sample) => {
            ...
            else if (sample.mode === 'extrapolated' && mode === 'interpolated')
                mode = 'extrapolated'
    ...
    private sendPingIfDue(now: number): void {
        ...
        this.transport.send(
            encodeMessage({ type: MessageType.Ping, payload: { pingId } })
        )
        this.nextPingAtMs = now + PING_INTERVAL_MS
    }
    private acceptPong(
        pong: import('../../protocol/generated').Pong,
        now: number
    ): void {
    ...
    private removeRemote(entityId: number): void {
        this.timelines?.remove(entityId)
        this.context?.services.get(ENTITY_VIEWS).removeAndDispose(entityId)
    }
    ...
    private handleClose(code: number, closeReason: string): void {
        ...
        if (this.status === 'reconnecting') {
            ...
            this.reconnectAtMs =
                ...
                Math.min(
                    5000,
                    500 * 2 ** Math.min(4, this.reconnectAttempts - 1)
                )
        }
    }
    private handleDecoderFailure(message: string): void {
        ...
        this.reconnectAtMs =
            performance.now() +
            Math.min(5000, 500 * 2 ** Math.min(4, this.reconnectAttempts - 1))
    }
    private fail(message: string): void {
        this.status = 'rejected'
        this.detail = message
        this.intentionalClose = true
        this.transport.close(1008, 'client validation failed')
        this.resetSession()
    }
    private resetSession(reason?: HardSyncReason): void {
        ...
        this.previousSnapshotAtMs = 0
        this.clock?.reset()
        this.clock = undefined
        this.interpolationDelay?.reset()
        this.interpolationDelay = undefined
        this.nextPingAtMs = 0
        this.interpolationMode = 'none'
        this.history.clear()
        this.timelines?.clear()
        ...
        this.sequence = 0
        this.actionId = 0x9e3779b9
        this.clientTick = 0
        this.clientTickAligned = false
        this.lastLocalFireAtMs = -Infinity
        this.pendingHardSyncReason = undefined
        this.clearVisualResidual()
        this.replaySteps = 0
        this.replayTimeMs = 0
        if (reason) this.recordHardSync(reason)
        this.context?.services.get(ENTITY_VIEWS).clearAndDispose()
        this.accumulator.reset()
        ConfigManager.resetConfig()
    }
    stop(): void {
    ...
    dispose(): void {
        this.stop()
        this.context?.services.get(PHYSICS).setExternalDrive(false)
        this.context?.services.remove(NETWORKING)
        this.context = undefined
    }
    private nextActionId(): number {
        this.actionId = (this.actionId + 1) >>> 0
        if (this.actionId === 0) this.actionId = 1
