...
export async function getLeaderboard(
  ...
  if (period === 'daily') {
    dateFilter = gte(playerStats.lastPlayedAt, sql`now() - interval '1 day'`);
  } else if (period === 'weekly') {
    dateFilter = gte(playerStats.lastPlayedAt, sql`now() - interval '7 days'`);
  }
  ...
  return results.map((row: { name: string; score: number | bigint; kills: number; wins: number; gamesPlayed: number }, index: number) => ({
    rank: index + 1,
    name: row.name,
