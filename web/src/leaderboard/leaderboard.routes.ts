...
import { z } from 'zod';
import { getLeaderboard, updatePlayerStats, LeaderboardPeriod } from './leaderboard.service';
...
const leaderboardRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    ...
    const query = request.query as { period?: LeaderboardPeriod; limit?: number };
    ...
    try {
      ...
      request.log.error({ error: error.message }, 'Failed to fetch leaderboard');
  ...
  fastify.post('/update', {
    ...
    try {
      ...
      const error = err as Error;
      request.log.error({ error: error.message }, 'Failed to update player stats');
      return reply.code(500).send({
