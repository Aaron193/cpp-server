...
import { z } from 'zod';
import { registerUser, authenticateUser, generateAccessToken, getUserById } from './auth.service';
...
const authRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/register', {
    schema: {
      body: z.object({
        username: z.string().min(CONSTANTS.MIN_USERNAME_LENGTH).max(CONSTANTS.MAX_USERNAME_LENGTH),
        email: z.string().email().optional(),
        password: z.string().min(CONSTANTS.MIN_PASSWORD_LENGTH).max(CONSTANTS.MAX_PASSWORD_LENGTH),
      }),
    },
  }, async (request, reply) => {
