...
class World {
   public:
    ...
    void saveTerrainMeshesJSON(const std::vector<TerrainMesh>& meshes,
                               const std::string& filename);

    // Box2D physics integration
    void BuildMeshPhysics(const std::vector<TerrainMesh>& meshes,
   ...
   private:
    ...
    void saveFloatImageAsGrayscale(const std::string& filename,
                                   const std::vector<float>& data);
    void saveColoredImage(const std::string& filename,
