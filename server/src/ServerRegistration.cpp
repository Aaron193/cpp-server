...
void ServerRegistration::registerServerAsync() {
    std::cout << "[ServerRegistration] Registering server with web API: " << m_webApiUrl << std::endl;
    ...
    std::cout << "[ServerRegistration] Registration payload: " << body << std::endl;
...
void ServerRegistration::sendHeartbeatAsync(int currentPlayers) {
    ...
    std::string body = payload.str();
    std::cout << "[ServerRegistration] Sending heartbeat: " << currentPlayers << " players" << std::endl;

    // Send async (non-blocking)
    sendHttpPostAsync("/servers/heartbeat", body);
}

void ServerRegistration::sendHttpPostAsync(const std::string& endpoint, const std::string& jsonBody) {
        ...
        try {
            ...
            if (res) {
                if (res->status == 200 || res->status == 201) {
                    std::cout << "[ServerRegistration] ✓ HTTP POST successful: " << path 
                              << " (status: " << res->status << ")" << std::endl;
                } else {
                    std::cerr << "[ServerRegistration] ✗ HTTP POST failed: " << path 
                              << " (status: " << res->status << ")" << std::endl;
                    std::cerr << "[ServerRegistration] Response: " << res->body << std::endl;
                }
            } else {
                auto err = res.error();
                std::cerr << "[ServerRegistration] ✗ HTTP request failed: " << path << std::endl;
                std::cerr << "[ServerRegistration] Error: " << httplib::to_string(err) << std::endl;
            }
