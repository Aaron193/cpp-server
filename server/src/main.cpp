...
int main() {
    ...
    std::cout << "[Config] Host: " << serverHost << ":" << serverPort << std::endl;
    ...
    if (!webApiUrl.empty() && !sharedSecret.empty()) {
        ...
        std::cout << "[Registration] Server registration initiated" << std::endl;
        ...
        std::cout << "[Registration] Heartbeat will be sent every " 
                  << gameServer.m_heartbeatInterval << " seconds from game loop" << std::endl;
    } else {
        std::cout << "[Registration] Skipping registration (WEB_API_URL or SERVER_SHARED_SECRET not set)" << std::endl;
    }
